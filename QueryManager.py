...
class Queries(ABC):
    ...
    def get_auto_increment_columns(schema, table):
        ...
        Returns columns that need sequence reset after data copy/insert.
        """
        pass

    @abstractmethod
    def generate_sequence_reset_sql(schema, table, column):
        """Generate SQL to reset a sequence for a SERIAL or IDENTITY column."""
        pass
...
class QueriesOracle(Queries):
    ...
    def get_auto_increment_columns(schema, table):
        ...
        Oracle 12c+ supports IDENTITY columns and DEFAULT ON NULL with sequences.
        Returns columns that need sequence reset after data copy/insert.
        """
        return f"""
    ...
    @staticmethod
    def generate_sequence_reset_sql(schema, table, column):
        """Generate PL/SQL block to reset an IDENTITY or sequence-based column.
        
        For IDENTITY columns, modifies the column to restart from max+1.
        For DEFAULT with SEQUENCE, the sequence continues automatically.
        """
        return f"""
            DECLARE
                v_max_val NUMBER;
    ...
    @staticmethod
    def reset_auto_increment_sequences(schema, table):
        """Generate PL/SQL block to reset all auto-increment sequences for a table.
        
        Returns a PL/SQL block that resets all IDENTITY and sequence-based columns.
        Returns empty string if no auto-increment columns exist.
        """
        return f"""
            DECLARE
                v_max_val NUMBER;
            ...
            BEGIN
                FOR rec IN (
                    ...
                    SELECT NVL(MAX("{rec.column_name}"), 0) + 1 INTO v_max_val FROM "{schema}"."{table}";
                    v_sql := 'ALTER TABLE "{schema}"."{table}" MODIFY "' || rec.column_name || '" GENERATED BY DEFAULT AS IDENTITY (START WITH ' || v_max_val || ')';
                    EXECUTE IMMUTE v_sql;
                END LOOP;
            END;
        """
...
class QueriesSQLite(Queries):
    ...
    def get_auto_increment_columns(schema, table):
        ...
        SQLite handles INTEGER PRIMARY KEY columns automatically as ROWID aliases.
        Only columns with explicit AUTOINCREMENT keyword need sequence management.
        Returns columns that might need sequence reset after data copy/insert.
        
        Note: SQLite's AUTOINCREMENT is different - it prevents reuse of deleted ROWIDs.
        For most cases, no reset is needed as SQLite auto-manages ROWID.
        """
        # Get columns that are INTEGER PRIMARY KEY (potential auto-increment)
        # Then check if the table SQL contains AUTOINCREMENT keyword
        return f"""
    ...
    def generate_sequence_reset_sql(schema, table, column):
        ...
        Most SQLite tables don't need reset - ROWID is auto-managed.
        """
        # Delete from sqlite_sequence to reset AUTOINCREMENT
        # This allows SQLite to reuse deleted ROWIDs
    ...
    def reset_auto_increment_sequences(schema, table):
        ...
        Returns SQL to delete from sqlite_sequence for the table.
        Empty string if no action needed (most SQLite tables).
        
        Note: SQLite handles INTEGER PRIMARY KEY automatically.
        Only tables with explicit AUTOINCREMENT keyword need this.
        """
...
class QueriesPostgreSQL(Queries):
    ...
    def get_auto_increment_columns(schema, table):
        ...
        PostgreSQL supports SERIAL pseudo-types (backed by sequences) and
    ...
    def generate_sequence_reset_sql(schema, table, column):
        ...
        Uses pg_get_serial_sequence() to find the sequence name and setval() to reset it.
        Works for both SERIAL pseudo-types and IDENTITY columns.
        """
        return f"""
            SELECT setval(
    ...
    def reset_auto_increment_sequences(schema, table):
        ...
        Returns a DO block that iterates over all serial/identity columns
        and resets their sequences to max+1.
        """
        return f"""
            DO $$
            DECLARE
            ...
            BEGIN
                FOR col_name IN (
                    ...
                    seq_name := pg_get_serial_sequence('{schema}.{table}', col_name);
                    IF seq_name IS NOT NULL THEN
                        EXECUTE format(
                            'SELECT setval(%L, COALESCE((SELECT MAX(%I) FROM %I.%I), 1), true)',
                            seq_name, col_name, '{schema}', '{table}'
                        );
                    END IF;
                END LOOP;
            END $$;
...
class QueriesMSSQL(Queries):
    ...
    @staticmethod
    def get_auto_increment_columns(schema, table):
        """Get list of IDENTITY columns in a table.
        
        SQL Server uses IDENTITY(seed, increment) for auto-increment columns.
        SEQUENCE objects are also supported but less common for simple auto-increment.
        Returns columns that need sequence reset after data copy/insert.
        """
        return f"""
    ...
    @staticmethod
    def generate_sequence_reset_sql(schema, table, column):
        """Generate DBCC CHECKIDENT to reset an IDENTITY column.
        
        DBCC CHECKIDENT with RESEED without a third parameter automatically
        calculates the new seed from MAX(column_value).
        """
        return f"DBCC CHECKIDENT ('[{schema}].[{table}]', RESEED);"

    @staticmethod
    def reset_auto_increment_sequences(schema, table):
        """Generate DBCC CHECKIDENT to reset all IDENTITY columns for a table.
        
        DBCC CHECKIDENT with RESEED (no explicit seed) automatically resets
